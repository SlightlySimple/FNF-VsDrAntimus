...
class ChartingState extends MusicBeatState
{
	...
	public var playClapsDad:Bool = false;
	...
	**/
	...
	var player2:Character = new Character(0, 0, "antimus");
	...
	override function create()
	{
		...
		zoomFactor = FlxG.save.data.editorZoom;
		...
		PlayState.noteskinSprite = NoteskinHelpers.generateNoteskinSprite(FlxG.save.data.noteskin, _song.noteStyle);
		...
		//var targetY = getYfromStrum(FlxG.sound.music.length);		// Why the fuck was this a thing in the first place

		//Debug.logTrace("TARGET " + targetY);

		for (awfgaw in 0...sections + 1) // grids/steps
		{
			var renderer = new SectionRender(0, 640 * awfgaw, GRID_SIZE, _song.fiveKey);
			if (_song.notes[awfgaw] == null)
			{
				if (awfgaw > 0)
					_song.notes.push(newSection(_song.notes[awfgaw-1].lengthInSteps, true, false, false));
				else
		...
		strumLine = new FlxSprite(0, 0).makeGraphic(Std.int(GRID_SIZE * (_song.fiveKey ? 10 : 8)), 4);
	...
	function regenerateLines()
	{
		...
		if (_song.eventObjects != null)
			for (i in _song.eventObjects)
			{
				...
				var line = new FlxSprite(0, pos).makeGraphic(Std.int(GRID_SIZE * (_song.fiveKey ? 10 : 8)), 4, FlxColor.BLUE);
		...
		for (i in sectionRenderes)
		{
			...
			var line = new FlxSprite(0, pos).makeGraphic(Std.int(GRID_SIZE * (_song.fiveKey ? 10 : 8)), 4, FlxColor.BLACK);
			line.alpha = 0.4;
			lines.add(line);
		}
	}
	...
	function addGrid(?divisions:Float = 1)
	{
		...
		remove(gridBG);
		gridBG = FlxGridOverlay.create(GRID_SIZE, Std.int(h), GRID_SIZE * (_song.fiveKey ? 10 : 8), GRID_SIZE * 16);
	...
	function addEventsUI()
	{
		...
		var eventList:Array<String> = CoolUtil.coolTextFile(Paths.txt('data/charting/eventList'));
	...
	{
		var hitsoundsBF = new FlxUICheckBox(10, 30, null, null, "Play hitsounds (Player 1)", 100);
		...
		var hitsoundsDad = new FlxUICheckBox(10, 60, null, null, "Play hitsounds (Player 2)", 100);
		hitsoundsDad.checked = false;
		hitsoundsDad.callback = function()
		{
			playClapsDad = hitsoundsDad.checked;
		};
		...
		tab_options.add(hitsoundsBF);
		tab_options.add(hitsoundsDad);
	...
	function addSongUI():Void
	{
		...
		var check_fiveKey = new FlxUICheckBox(10, 40, null, null, "Five key", 100);
		check_fiveKey.checked = _song.fiveKey;
		check_fiveKey.callback = function()
		{
			_song.fiveKey = check_fiveKey.checked;
			addGrid(1);
			updateGrid();
			for (i in sectionRenderes)
			{
				i.reRender(_song.fiveKey);
			}
		};
		...
		var characters:Array<String> = CoolUtil.coolTextFile(Paths.txt('data/charting/characterList'));
		var gfVersions:Array<String> = CoolUtil.coolTextFile(Paths.txt('data/charting/gfVersionList'));
		var stages:Array<String> = CoolUtil.coolTextFile(Paths.txt('data/charting/stageList'));
		var noteStyles:Array<String> = CoolUtil.coolTextFile(Paths.txt('data/charting/noteStyleList'));
		...
		tab_group_song.add(check_fiveKey);
		...
		camFollow = new FlxObject(280 + (GRID_SIZE * 2), 0, 1, 1);
		add(camFollow);
	...
	function addSectionUI():Void
	{
		...
		stepperLength = new FlxUINumericStepper(10, 10, 4, 16, 4, 256);
		stepperLength.value = _song.notes[curSection].lengthInSteps;
		stepperLength.name = 'section_length';
		var stepperLengthLabel = new FlxText(stepperLength.x + stepperLength.width + 10, 10, 'Section Length');
	...
	{
		var actualSection:SwagSection = _song.notes[section];
		var beat = TimingStruct.getBeatFromTime(actualSection.startTime);
	...
	function addNoteUI():Void
	{
		...
		check_naltAnim = new FlxUICheckBox(10, 150, null, null, "Reverse Scroll", 100);
		check_naltAnim.callback = function()
		{
			if (curSelectedNote != null)
			{
				for (i in selectedBoxes)
				{
					...
					{
						if (n.strumTime == i.connectedNoteData[0] && n.rawNoteData == i.connectedNoteData[1])
							n.downscroll = i.connectedNoteData[3];
					}
				}
			}
		}
		...
		for (i in array)
		{
			...
			for (ii in _song.notes)
			{
				if (ii.startTime <= strum && ii.endTime > strum)
				{
					...
					var note:Note = new Note(strum, Math.floor(i[1] % (_song.fiveKey ? 5 : 4)), null, false, true, i[3]);
		...
		for (i in 0...selectedBoxes.members.length)
		{
			...
			for (ii in _song.notes)
			{
				if (ii.startTime <= strum && ii.endTime > strum)
				{
					...
					if (originalNote.downscroll)
						newData.push(originalNote.downscroll);
					ii.sectionNotes.push(newData);

					var thing = ii.sectionNotes[ii.sectionNotes.length - 1];

					var note:Note = new Note(strum, originalNote.noteData, originalNote.prevNote, originalNote.isSustainNote, true, originalNote.downscroll);
	...
	function poggers()
	{
		...
		for (section in _song.notes)
		{
			...
			for (note in section.sectionNotes)
			{
				// commit suicide
				var old = note[0];
				note[0] = Math.round(TimingStruct.getTimeFromBeat(TimingStruct.getBeatFromTime(note[0])));
				note[2] = Math.round(TimingStruct.getTimeFromBeat(TimingStruct.getBeatFromTime(note[2])));
				// All these section start and end times are offset by 10ms to account for possible incorrect rounding of the note positions
				if (note[0] < Math.round(section.startTime) - 10)
				{
					notes.push(note);
					removed.push(note);
				}
				if (note[0] >= Math.round(section.endTime) - 10)
				{
					notes.push(note);
					removed.push(note);
				}
		...
		for (section in _song.notes)
		{
			...
			for (i in notes)
			{
				if (i[0] >= Math.round(section.startTime) - 10 && i[0] < Math.round(section.endTime) - 10)
				{
					saveRemove.push(i);
	...
	{
		var noteCount = (_song.fiveKey ? 5 : 4);
		...
		for (i in 0...secit.sectionNotes.length)
		{
			...
			var n = [note[0], Std.int(note[1]), note[2]];
			if (note[3])
				n.push(note[3]);
			n[1] = (note[1] + noteCount) % (noteCount * 2);
			newSwaps.push(n);
		}
		...
		for (i in shownNotes)
		{
			for (ii in secit.sectionNotes)
				if (i.strumTime == ii[0] && i.noteData == ii[1] % noteCount)
				{
		...
		try
		{
			...
			if (doInput)
			{
				...
				if (FlxG.keys.pressed.CONTROL && FlxG.keys.justPressed.C)
				{
					if (selectedBoxes.members.length != 0)
					{
						...
						{
							if (i.connectedNote.downscroll)
								copiedNotes.push([i.connectedNote.strumTime, i.connectedNote.rawNoteData, i.connectedNote.sustainLength, i.connectedNote.downscroll]);
							else
			...
			if (weird != null)
			{
				if (lastUpdatedSection != getSectionByTime(start, true))
				{
					...
					stepperLength.value = weird.lengthInSteps;
				}
			}
			...
			var cock = FlxG.keys.justPressed.NINE;
			var balls = FlxG.keys.justPressed.ZERO;
			...
			if (_song.fiveKey)
			{
				pressArray.push(cock);
				pressArray.push(balls);
			}
			var delete = false;
			if (doInput)
			{
			...
			if (playClapsBF || playClapsDad)
			{
				for (note in shownNotes)
				{
					if (note.strumTime <= Conductor.songPosition && !claps.contains(note) && FlxG.sound.music.playing)
					{
						var section = getSectionByTime(note.strumTime + 10);
						var countThisNote = true;
						var noteCount = (_song.fiveKey ? 5 : 4);
						if (section.mustHitSection)
						{
							if (note.rawNoteData >= noteCount && !playClapsDad)
								countThisNote = false;
							if (note.rawNoteData < noteCount && !playClapsBF)
								countThisNote = false;
						}
						else
						{
	...
	function updateGrid():Void
	{
		...
		for (section in _song.notes)
		{
			for (i in section.sectionNotes)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % (_song.fiveKey ? 5 : 4), null, false, true, i[3]);
	...
	private function newSection(lengthInSteps:Int = 16, mustHitSection:Bool = false, CPUAltAnim:Bool = true, playerAltAnim:Bool = true):SwagSection
	{
		...
		var currentBeat = lengthInSteps / 4;

		for (i in _song.notes)
			currentBeat += i.lengthInSteps / 4;
	...
	private function addNote(?n:Note):Void
	{
		...
		{
			if (n.downscroll)
				section.sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.downscroll]);
			else
		...
		if (n == null)
		{
			var note:Note = new Note(noteStrum, noteData % (_song.fiveKey ? 5 : 4), null, false, true);
		...
		{
			var note:Note = new Note(n.strumTime, n.noteData % (_song.fiveKey ? 5 : 4), null, false, true, n.downscroll);
	...
	{
		PlayState.SONG = Song.loadFromJson(songId, PlayState.storyDifficulty);
	...
	function loadAutosave():Void
	{
		...
		var meta:SongMeta = {difficulties: [PlayState.storyDifficulty]};
		var name:String = data.songId;
		if (autoSaveData.song != null)
		{
			meta = autoSaveData.songMeta != null ? cast autoSaveData.songMeta : {difficulties: [PlayState.storyDifficulty]};
		}
	...
	{
		var filename:String = _song.songId.toLowerCase() + "-" + PlayState.storyDifficulty.toLowerCase() + ".json";
		if (PlayState.storyDifficulty.toLowerCase() == "normal")
			filename = _song.songId.toLowerCase() + ".json";
