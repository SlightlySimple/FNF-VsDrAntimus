...
class Character extends FlxSprite
{
	...
	public var eyeglow:Character = null;
	...
	function parseDataFile()
	{
		...
		if (data.eyeglow != null)
			eyeglow = new Character(0, 0, data.eyeglow, isPlayer);
		...
		if (Paths.doesTextAssetExist(Paths.file('data/characters/' + curCharacter + '.hs')))
		{
			myScript = new HscriptHandler('characters/' + curCharacter);
			myScript.setVar('char', this);
	...
	override function update(elapsed:Float)
	{
		...
		if ( eyeglow != null )
		{
			...
			if ( eyeglow.animation.curAnim.name != animation.curAnim.name )
				eyeglow.playAnim( animation.curAnim.name, true );
			eyeglow.animation.curAnim.curFrame = animation.curAnim.curFrame;
		}
	...
	public function dance(forced:Bool = false, altAnim:Bool = false)
	{
		if (!debugMode && !importantAnimation)
		{
			switch (danceType)
			{
				...
				case 'scripted':
					if (myScript != null)
						myScript.execFunc('dance', [forced, altAnim]);
