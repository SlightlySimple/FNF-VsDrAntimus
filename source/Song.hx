...
class Song
{
	...
	{
		var songFile = '$songId/$songId-' + difficulty.toLowerCase();
		if (difficulty.toLowerCase() == 'normal')
			songFile = '$songId/$songId';
	...
	public static function parseJSONshit(songId:String, jsonData:Dynamic, jsonMetaData:Dynamic):SongData
	{
		...
		if (songData.eventObjects == null && songMetaData.eventObjects != null)
			songData.eventObjects = songMetaData.eventObjects;

		if (songData.length == 0 && songMetaData.length != 0)
			songData.length = songMetaData.length;

		if (jsonData.song.fiveKey == null)
			songData.fiveKey = songMetaData.fiveKey;
