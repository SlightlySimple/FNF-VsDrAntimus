...
class Ratings
{
	...
	public static function CalculateRanking(score:Int, scoreDef:Int, nps:Int, maxNPS:Int, accuracy:Float):String
	{
		return (FlxG.save.data.npsDisplay ? // NPS Toggle
			CoolUtil.translate('ratingNPS', [['{nps}', '' + nps], ['{maxNPS}', '' + maxNPS]])
			+ (!PlayStateChangeables.botPlay || PlayState.loadRep ? " | " : "") : "") + // 	NPS
			(!PlayStateChangeables.botPlay
				|| PlayState.loadRep ? CoolUtil.translate('ratingScore', [['{score}', (Conductor.safeFrames != 10 ? score + " (" + scoreDef + ")" : "" + score)]]) + // Score
					(FlxG.save.data.accuracyDisplay ? // Accuracy Toggle
						" | " + CoolUtil.translate('ratingMisses', [['{misses}', '' + PlayState.misses]]) + // 	Misses/Combo Breaks
						" | " + CoolUtil.translate('ratingAccuracy', [['{accuracy}', '' + HelperFunctions.truncateFloat(accuracy, 2) + " %"]]) + // 	Accuracy
